...
def sign_url(url, key, secret):
    ...
    signature = base64.b64encode(hmac.new(secret.encode(), url.encode(), sha1).digest()).decode()
...
short_label = sign_url('http://webservices.esd.org.uk/areas?searchText=sw11&', 'OxbQLXXqwAwghwvmycrbHtIgHGdFPhZlbvakqHyW', 'WPgIAQmXypXmKckAEdhf')


def getLA(searchText):
    searchText = searchText.replace(" ", "")
    return sign_url(f'http://webservices.esd.org.uk/areas?searchText={searchText}&', 'OxbQLXXqwAwghwvmycrbHtIgHGdFPhZlbvakqHyW', 'WPgIAQmXypXmKckAEdhf')
